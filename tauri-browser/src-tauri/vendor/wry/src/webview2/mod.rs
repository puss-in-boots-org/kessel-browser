  ...
  unsafe fn attach_ipc_handler(
    ...
    webview.add_WebMessageReceived(
      &WebMessageReceivedEventHandler::create(Box::new(move |_, args| {
        ...
        // KESSEL PATCH (see vendor/wry/KESSEL-PATCH.md): nothing below may
        // return an error or panic. An error makes WebView2 skip the
        // event's other handlers, and a panic here aborts the app.
        let url = {
          let mut url = PWSTR::null();
          if args.Source(&mut url).is_err() {
            return Ok(());
          }
          take_pwstr(url)
        };

        // Not a string: not an IPC message -- leave it to the other handlers.
        let js = {
        ...
        // A page whose address isn't a valid http::Uri (file:, about:,
        // data:, blob:, longer than 64 KB) can't be an IPC caller either.
